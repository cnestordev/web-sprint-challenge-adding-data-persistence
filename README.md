...
A relational database stores data in the form of tables. SQL allows you to interact with the relational database by making queries.

2. Why do tables need a `primary key`?
One reason being that they can be identified by other tables that link to them, and another is to find it by its id when querying.

3. What is the name given to a table column that references the primary key on another table.
Foreign id.

4. What do we need in order to have a _many to many_ relationship between two tables.
We will need a third, intermediate (join table) table that links the two.  This table can include additional data that related to both tables.
